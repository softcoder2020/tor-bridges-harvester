...
func loadRelays(urls []string, timeout time.Duration, proxy string) ([]Relay, error) {
	...
	for _, u := range urls {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "Can't download Tor Relay data from %s: %v\n", u, err)
			continue
		}
		...
		if err := json.NewDecoder(resp.Body).Decode(&data); err != nil {
			fmt.Fprintf(os.Stderr, "Failed to parse JSON from %s: %v\n", u, err)
			continue
		}
		fmt.Fprintf(os.Stderr, "Successfully loaded relays from %s\n", u)
...
// generateOutput writes the relay configuration to outfile and appends to bridges_grok.txt
	...
	for _, r := range workingRelays {
		for _, addr := range r.Reachable {
			...
			if outfile != os.Stdout {
				fmt.Fprintf(os.Stderr, "Added to output: %s", line)
			}
		}
	}
	if torrcFmt {
...
func startBrowser() error {
	...
	for _, cmd := range browserCmds {
		...
		if _, err := os.Stat(parts[0]); err == nil {
			...
			if err != nil {
				fmt.Fprintf(os.Stderr, "Failed to start browser with %s: %v\n", cmd, err)
				continue
			}
			fmt.Fprintf(os.Stderr, "Successfully started browser with %s\n", cmd)
			return nil
		}
	}
...
func main() {
	...
	fmt.Fprintf(os.Stderr, "Tor Relay Scanner. Will scan up to %d working relays\n", *goal)
	fmt.Fprintln(os.Stderr, "Downloading Tor Relay information from Tor Metrics…")
	...
	if err != nil {
		log.Fatalf("Failed to open _bridges.txt: %v", err)
	}
	...
	for i := 0; i < len(relays) && len(workingRelays) < *goal; i += *numRelays {
		...
		for _, r := range chunk {
			fmt.Fprintln(os.Stderr, r.Fingerprint)
		}
		...
		fmt.Fprintln(os.Stderr, "Reachable relays this attempt:")
		for _, r := range chunk {
			if len(r.Reachable) > 0 {
				for _, addr := range r.Reachable {
	...
	if len(workingRelays) > 0 {
		...
		fmt.Fprintln(os.Stderr, "No reachable relays found.")
	}
